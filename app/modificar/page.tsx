...
export default function ModificarPage() {
  ...
  return (
    ...
    {mensaje && <p className="status-message">{mensaje}</p>}<div className="action-row flex items-center gap-3 mt-6"><button className="secondary-button" onClick={() => window.location.href = `/?token=${encodeURIComponent(token)}`}><ArrowLeft size={18} /> Volver</button><button className="primary-button ml-auto" onClick={enviar} disabled={enviando || cargando || !plantelId}>{enviando ? <><LoaderCircle className="loading-icon" size={18} /> Enviando...</> : <><Send size={18} /> Registrar modificación</>}</button></div></>}</div></section></div></main>
  );
}
